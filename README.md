...
Pre-built patched binaries (Windows x64) are available on the [Releases page](https://github.com/bigjakk/Electron-Websocket-Fix/releases):
...
The patch modifies Chromium source (not Electron source), so it applies to any Electron version since Electron 10 (Chromium 84+). Line numbers may shift between versions but the function names remain the same:
